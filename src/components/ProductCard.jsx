...
export default function ProductCard({ p, onDelete, onToggleFavProp }) {
    ...
    const fetchFav = async () => {
      try {
        const res = await axios.get(
          `https://re-style-backend.vercel.app/api/product-actions/my/favorites` /*`http://localhost:3000/api/product-actions/my/favorites` */,
  ...
  const handleLike = async (e) => {
    ...
    try {
      const res = await axios.post(
        `https://re-style-backend.vercel.app/api/product-actions/${p._id}/like` /* `http://localhost:3000/api/product-actions/${p._id}/like` */,
        {},
        { withCredentials: true }
      );
  ...
  const handleToggleFav = async (e) => {
    ...
    try {
      const res = await axios.post(
         `https://re-style-backend.vercel.app/api/product-actions/${p._id}/favorite` /*`http://localhost:3000/api/product-actions/${p._id}/favorite` */ ,
        {},
        { withCredentials: true }
      );
      ...
      setIsFav(!optimistic); // revert if backend fails
      if (onToggleFavProp) onToggleFavProp(p._id, !optimistic);
    }
  };
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const res = await axios.post(
        `https://re-style-backend.vercel.app/api/product-actions/${p._id}/comment`  /* `http://localhost:3000/api/product-actions/${p._id}/comment` */,
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      const res = await axios.delete(
        `https://re-style-backend.vercel.app/api/product-actions/${p._id}/comment/${commentId}`  /* `http://localhost:3000/api/product-actions/${p._id}/comment/${commentId}` */,
        { withCredentials: true }
      );
      setComments(res.data.comments);
    } catch (err) {
    ...
    <div className="border rounded overflow-hidden shadow-sm bg-white hover:shadow-lg transition-shadow cursor-pointer">
      ...
      <div className="p-3">
        ...
        <button
          ...
          className={`mt-3 w-full ${isInCart ? "bg-red-600" : "bg-blue-600"} text-white py-2 rounded hover:bg-opacity-80 transition`}
        >
        ...
        <div className="flex items-center gap-3 mt-3">
          <button
            onClick={handleLike}
            className={`px-3 py-1 rounded ${liked ? "bg-pink-500 text-white" : "bg-gray-200 text-gray-700"} hover:bg-pink-600`}
          >
            👍 Like {likesCount}
          </button>
          <button
            ...
            <div className="bg-white p-6 rounded shadow-lg w-96 relative">
              ...
              <div className="max-h-60 overflow-y-auto mb-2">
                {comments.length === 0 ? (
                  ...
                  comments.map((c) => (
                    <div key={c._id || c.createdAt} className="mb-2 text-gray-800 border-b pb-1">
